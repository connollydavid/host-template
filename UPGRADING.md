...
[upgrade "RETIRE-hermetic-exempt"]
    ...
    action   = Entry `ecce498` told you that a component which cannot vendor offline "may carry `hermetic-exempt = call/NNNN`". No release of host-lifecycle ever parsed that key: a recipe carrying it is read as if the line were absent, so an adopter who wrote it has an exemption that never applied and a gate that stayed red. Remove any `hermetic-exempt` line from your `.host-software`, and record the case with `repro-exempt = call/NNNN` instead, citing the same decision. Hermeticity is a facet of reproducibility rather than a second property, so one key carries both and the two can no longer disagree. Nothing else changes. The property MUST stands: reproduce release binaries offline from pinned inputs. The gate invariant stands too, so a component that records a `deps-bundle` still builds with no network and its staged bundle hash still matches the recorded one. A project that never wrote the key needs no action. Recorded as agentic-host call/0046.
    requires = host-lifecycle v0.20.0
    independent = true
